...
public class DrawMapRandomColors{
...
//    private static double[] Xpts, Ypts;   // the points (x[i], y[i])
    ...
    public static int[] aryRGB; //RGB values
    public static String[] aryCountyNames=new String[3210];
    public static int[][] aryCountyColors=new int[3210][3];
        ...
        if (stateName.equals(previousStateName)){
          ...
          if (fileName.equals("USA-county.txt")){  //first state in USA-county.txt
            if (testCountryName.equals(previousStateName)){   // ("AL")){
              ...
              int yearEntered=2012;//will replace with year entered by user in the menu
            loadRGBCountyArray(stateAbbreviation,yearEntered);//method to calculate purple color for each state for the year user enters
              stateNumber+=1;
            }
          }else{
            ...
            stateAbbreviation=aryStateAbbrev[stateNumber];
            int yearEntered=2012;//will replace with year entered by user in the menu
            loadRGBStateArray(stateAbbreviation,yearEntered);//method to calculate purple color for each state for the year user enters
            stateNumber+=1;
          }
        }  
          if (fileName.equals("USA-county.txt")){  // USA-county.txt
            drawCounty(aryLongX, aryLatY, stateAbbreviation, stateName); //stateName is CountyName    //drawState() method calls StdDraw.filledPolygon() AND StdDraw.polygon() in black for border 
          }else{  
...
//      //StdDraw.setPenColor(StdDraw.BLUE);                 //1 Color version of PenColor method
...
//      public static void fill() { StdDraw.filledPolygon(Xpts, Ypts); }
      public static int[] getRGBCountyColor(String CountyName) {
        int[] aryReturnVal = new int[3] ;
       // for(int x=0; x<3; x++){
       // aryReturnVal[x] = (int)(Math.random() * 255 + 1);  // 0 to 255 based on votes
       // }
        int x=0;
        String delims = "[,]";
        boolean stillSearching=true;
        while(stillSearching==true){
          if (aryCountyNames[x].equals("")){   //value is blank so no more county names
            System.out.println("parameter CountyName="+CountyName+" aryCountyNames value"+aryCountyNames[x]); //debug show variable on console
            for(int z=0; z<3; z++){
              aryReturnVal[z] =  255;  // return 255,255,255
            }
            stillSearching=false;
          }else{

          String[] countyFields = aryCountyNames[x].split(delims);
          if (countyFields[0].equals(CountyName)){
            int a1 = Integer.parseInt(countyFields[1]);  
            int a2 = Integer.parseInt(countyFields[2]);  
            int a3 = Integer.parseInt(countyFields[3]);
            int voteTotal = a1+a2+a3;
            for(int z=0; z<3; z++){
            aryReturnVal[z] = (int) ((Integer.parseInt(countyFields[z+1])/voteTotal* 255 + 1));  // 0 to 255 based on votes
            }
              stillSearching=false;
            }else{
              stillSearching=true;
      ...
      public static void loadRGBCountyArray(String stateAbbreviation, int yearEntered)throws Exception { //method to calculate purple color for each state for the year user enters
       String voteFileName = stateAbbreviation+String.valueOf(yearEntered)+".txt";
        File voteIn = new File("src\\data\\" + voteFileName);   
      Scanner voteScan = new Scanner(voteIn);
        int x=0;
        String delims = "[,]";
        while(voteScan.hasNext()){
            aryCountyNames[x]=voteScan.nextLine();
            String[] countyFields = aryCountyNames[x].split(delims);
            //System.out.println("aryCountyNames value"+aryCountyNames[x]); //debug show variable on console
            //System.out.println("CountyFld1 value"+countyFields[0]+countyFields[1]+countyFields[2]+countyFields[3]); //debug show variable on console
            x++;
        }
      }
      
      public static void loadRGBStateArray(String stateAbbreviation, int yearEntered)throws Exception { //method to calculate purple color for each state for the year user enters
        String voteFileName = stateAbbreviation+String.valueOf(yearEntered)+".txt";//File file = new File("src\\data\\" + fileName);
          File voteIn = new File("src\\data\\" + voteFileName);   
      Scanner voteScan = new Scanner(voteIn);  
      String[] aryCountyNames=new String[3210];
        int[][] aryCountyColors=new int[3210][3];  
        while(voteScan.hasNext()){
        }
      }
...
public static void setupMap() {
       ...
       public static void drawCounty(double[] aryLongX, double[] aryLatY, String stateAbbreviation, String countyName) {
     ...
     //    StdDraw.setPenColor(250, 100, 100);    //write a getColor method to set RGB based on votes
         aryRGB = getRGBCountyColor(countyName);   //getRGB value for county color
         StdDraw.setPenColor(aryRGB[0],aryRGB[1],aryRGB[2]);    //write a getColor method to set RGB based on votes
         StdDraw.filledPolygon(aryLongX, aryLatY);   //draw polygon using pts stored in arrays
